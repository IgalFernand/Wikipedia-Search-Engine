...
helper = Helper()
TITLE_INDEX = get_pickle('/home/igalfernand/postings_gcp/title/index_title.pkl')
TEXT_INDEX = get_pickle('/home/igalfernand/postings_gcp/text/index_text.pkl')
ANCHOR_INDEX = get_pickle('/home/igalfernand/postings_gcp/anchor/index_anchor.pkl')
...
def search():
    ...
    ranking_results_docs_ids = helper.frequency_ranking(query_tokens, inverted_index=TITLE_INDEX, folder='/home/igalfernand/postings_gcp/title/', n=100)
    page_ranks_by_id = {doc_id: helper.get_page_rank_by_id(doc_id) for doc_id in ranking_results_docs_ids}
    sorted_ids_by_pr = sorted(page_ranks_by_id.items(), key=lambda item: item[1], reverse=True)
    res = helper.get_doc_title_pairs_from_items(sorted_ids_by_pr)
    return jsonify(res)
...
def search_title():
    ...
    query_tokens = helper.get_tokens(query)
    ranking_results_docs_ids = helper.frequency_ranking(query_tokens, inverted_index=TITLE_INDEX, folder='/home/igalfernand/postings_gcp/title/')
    res = helper.get_doc_title_pairs_from_id(ranking_results_docs_ids)
...
def search_anchor():
    ...
    if len(query) == 0:
        return jsonify(res)
    query_tokens = helper.get_tokens(query)
    ranking_results_docs_ids = helper.frequency_ranking(query_tokens, inverted_index=ANCHOR_INDEX, folder='/home/igalfernand/postings_gcp/anchor/')
    res = helper.get_doc_title_pairs_from_id(ranking_results_docs_ids)
