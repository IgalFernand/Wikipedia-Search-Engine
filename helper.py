...
class Helper:
    ...
    def frequency_ranking(self, tokens, inverted_index, folder, n=None):
        """
        ...
        sorted_docs_id_by_how_many_words_of_the_query_in_it = sorted(terms_in_doc.items(), key=lambda item: item[1],
                                                                     reverse=True)
        # Saving only the doc id's
        res = [item[0] for item in sorted_docs_id_by_how_many_words_of_the_query_in_it]
        if n:
            return res[:n]
        else:
            return res
    ...
    def get_candidate_documents_and_scores(self, query_to_search, inverted_index, words, pls):
        ...
        for term in np.unique(query_to_search):
            if term in words:
                ...
                normalized_tfidf = [(doc_id, (freq / inverted_index.DL[doc_id]) * math.log(len(inverted_index.DL) / inverted_index.df[term], 10)) for
    ...
    def get_doc_title_pairs_from_items(self, res):
        ...
        """
        return [(item[0], self.TITLES[item[0]]) for item in res]
    ...
    def get_doc_title_pairs_from_id(self, res):
        ...
        :return: list of tuples, (wiki id, title).
        """
        return [(id, self.TITLES[id]) for id in res]